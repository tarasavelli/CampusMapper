...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
          ...
          // initializing the start to be the first node in copyPath, the end node to be extendBy's target node and the distance the sum of the weights of the extend edge and the path
          this.start = copyPath.start;
          this.end = extendBy.target;
          this.distance = copyPath.distance + extendBy.weight;
          
          // manually copying to maintain independence of data sequences between paths
          this.dataSequence = new LinkedList<>();
          
          for(T element : copyPath.dataSequence)
            this.dataSequence.add(element);
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
      ...
      // hash table that contains the shortest path to each node from the start, key is the data stored in the end node of the path
      Hashtable<T, Path> shortestPaths = new Hashtable<T, Path>();
      shortestPaths.put(this.vertices.get(start).data, new Path(this.vertices.get(start)));
      
      PriorityQueue<Path> orderedPaths = new PriorityQueue<Path>();
      LinkedList<Vertex> visited = new LinkedList<Vertex>();
      
      // adding start vertex to the priority queue
      ...
      while(!orderedPaths.isEmpty()) {
        ...
        for(Edge e : current.end.edgesLeaving) {
          Path testPath = new Path(current, e);
          if(shortestPaths.containsKey(e.target.data)) {
            if(testPath.distance < shortestPaths.get(e.target.data).distance) {
              shortestPaths.put(e.target.data, testPath);
              orderedPaths.add(testPath);
            }
          }
          else {
            shortestPaths.put(e.target.data, testPath);
